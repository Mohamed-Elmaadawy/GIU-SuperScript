...
(function () {
    ...
    function render(sessions, opts = {}) {
        ...
        // Exam name + (control room when supervising) + duty type — hall is promoted to the title.
        const metaLine = s => `${s.examName}${s.type === 'Supervisor' ? ` · Control Room ${s.controlRoom}` : ''} · ${s.type}`;

        const nextCard = s => `
            <div class="gius-pr-next-card" data-idx="${upcoming.indexOf(s)}">
                <div class="gius-pr-next-title">${dutyLabel(s)}: ${s.courseCode} · Hall ${s.hall} ${badge(s)}</div>
        ...
        const nextHtml = `
            <div id="gius-pr-next" class="gius-pr-next">
                <div class="gius-pr-next-head">${fmtDateOnly(next.start)} · <b>${relTime(next.start, now)}</b>${nextDay.length > 1 ? ` · ${nextDay.length} duties` : ''}</div>
        ...
        const listHtml = remaining.length ? `
            <button type="button" id="gius-pr-toggle-all" class="gius-pr-toggle">Remaining exams (${remaining.length})</button>
            <div id="gius-pr-all" class="gius-pr-expand-wrapper">
                <div class="gius-pr-expand-inner">
                    <div class="gius-pr-all">
                        ${remaining.map(s => `
                            <div class="gius-pr-row" data-idx="${upcoming.indexOf(s)}">
                                <div class="gius-pr-title">${dutyLabel(s)}: ${s.courseCode} · Hall ${s.hall} ${badge(s)}</div>
                                <div class="gius-pr-meta">${fmtHuman(s.start)} – ${fmtTimeOnly(s.end)} · ${s.examName}</div>
                                <div class="gius-pr-actions"></div>
                            </div>`).join('')}
                    </div>
                </div>
