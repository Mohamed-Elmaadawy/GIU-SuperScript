...
(function () {
    ...
    async function fetchDepartment(dept, baseState) {
        ...
        return Array.from(table.querySelectorAll('a')).map(a => {
            ...
            return match ? { name: a.textContent.trim().replace(/^(Miss|Ms\.|Mrs\.|Mr\.|Dr\.|Prof\.|Eng\.)\s*/i, ''), target: match[1], deptLabel: dept.label, state: deptState } : null;
        }).filter(Boolean);
    }
    ...
    async function fetchProctorSchedule(proctor) {
        ...
        Array.from(table.querySelectorAll('tr')).forEach((tr, i) => {
            ...
            const coverName = tds[4].textContent
                .replace(/ /g, ' ')
                .replace(/^(Miss|Ms\.|Mrs\.|Mr\.|Dr\.|Prof\.|Eng\.)\s*/i, '')
                .trim();
    ...
    function injectStyles() {
        ...
        s.textContent = `
            ...
            .gius-cover-view-row td:first-child { font-style: italic; }
            .gius-cover-view-row { border-left: 3px solid #6366f1; }
            html.gius-dark .gius-cover-view-row { border-left-color: #818cf8; }
    ...
    function applyFiltersAndSort() {
        ...
        let rows = _allRows.flatMap(r => {
            if (dayQ     && !r.date.toLowerCase().includes(dayQ))           return [];
            if (examQ    && !r.examName.toLowerCase().includes(examQ))      return [];
            if (roomQ    && !r.hall.toLowerCase().includes(roomQ))          return [];
            if (deptQ    && !r.department.toLowerCase().includes(deptQ))    return [];
            if (search) {
            ...
            if (proctorQ) {
                const matchProctor = r.proctor.toLowerCase().includes(proctorQ);
                const matchCover   = r.coverName.toLowerCase().includes(proctorQ);
    ...
    function exportCSV() {
        ...
        const rows = _allRows.filter(r => {
            ...
            if (proctorQ && !r.proctor.toLowerCase().includes(proctorQ) && !r.coverName.toLowerCase().includes(proctorQ)) return false;
    ...
    function toggleCoProctors(tbody, tr, row) {
        ...
        if (row._coverView) {
            isActingCover = true;
            const origRows = _allRows.filter(r =>
                r.proctor  === row.proctor &&
                r.hall     === row.hall &&
                r.dateKey  === row.dateKey &&
                r.examName === row.examName
            );
            coProctors = origRows.length ? origRows : [{ proctor: row.proctor, department: row.department }];
        ...
        const cpItems = coProctors.map(r => {
            ...
            else if (r.coverName === row.proctor) badge = `<span class="gius-original-status">Original Proctor</span>`;
            return `<li>${escHtml(r.proctor)}${dept}${badge}</li>`;
        }).join('');
        const label = isActingCover ? 'Covering for:' : 'Co-proctors:';
        const icon  = isActingCover ? 'fa-exchange' : 'fa-users';
