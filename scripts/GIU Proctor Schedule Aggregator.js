...
(function () {
    ...
    function toggleCoProctors(tbody, tr, row) {
        ...
        const cpItems = coProctors.map(r => {
            ...
            const badge = isCovers ? `<span class="gius-cover-status">Cover Proctor</span>` : '';
            return `<li>${escHtml(r.proctor)}${dept}${badge}</li>`;
        }).join('');
        cpRow.innerHTML = `<td colspan="7"><span class="gius-coproctor-label"><i class="fa fa-users"></i> Co-proctors:</span><ul class="gius-coproctor-list">${cpItems}</ul></td>`;
