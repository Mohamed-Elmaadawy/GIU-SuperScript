...
(function () {
    ...
    function stripTitle(s) { return s ? s.replace(/ /g, ' ').replace(TITLE_RE, '').trim() : s; }
    function normalizeRow(r) {
        return (r.proctor && TITLE_RE.test(r.proctor)) || (r.coverName && TITLE_RE.test(r.coverName))
            ? { ...r, proctor: stripTitle(r.proctor), coverName: stripTitle(r.coverName) }
            : r;
    }
    ...
    function wirePanel(panel) {
        ...
        fileInput?.addEventListener('change', () => {
            ...
            reader.onload = ev => {
                ...
                _scraping = false;
                _paused   = true;
                _allRows  = rows.map(normalizeRow);
                saveCache(_allRows);
    ...
    function openPanel() {
        ...
        if (cached && cached.rows && cached.rows.length) {
            _allRows = cached.rows.map(normalizeRow);
