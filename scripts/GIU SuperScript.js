// ==UserScript==
// @name        GIU SuperScript
// @description All-in-one GIU staff portal enhancements (8 features). Install this OR the individual scripts — never both.
// @author      Mo.Elmaadawy
...
/*
 * GIU SuperScript — all-in-one bundle of 8 portal features.
 *   Dark Mode ships separately (needs @run-at document-start; FOUC-sensitive).
 *
 * ⚠ INSTALL THIS *OR* THE INDIVIDUAL SCRIPTS — NEVER BOTH.
 *   Both match the same pages → double widgets / double handlers.
 *
 * Architecture (see ARCHITECTURE notes below):
 *   CONFIG  — per-feature toggles, persisted in localStorage.
 *   Shared  — single impl of utils duplicated across the old scripts.
 *   ROUTES  — path test per feature; array order is deterministic (fixes Home.aspx race).
 *   Features— one module per feature, receives `Shared`.
 *   bootstrap — toggle check → path test → per-feature try/catch (one crash ≠ all dead).
 *
 * Per-feature internal versions (bundled-from sources):
 *   staffAttendance   3.1.3
 *   uploadGrades      2.4
 *   teachingLoad      1.1
 *   proctorReminder   1.1
 *   proctorAggregator 2.1
 *   notificationBatch 1.4
 *   manageGroupGrades 1.5
 *   studentAttendance (from GIU Student Attendance Report)
 */
...
(function () {
    ...
    const FEATURE_LABELS = {
        ...
        proctorAggregator: 'Proctor Aggregator',
    ...
    const Shared = {
        ...
        // Populated incrementally as features are folded in (Catppuccin Mocha-ish).
        // Each fold contributes the exact tokens its source script used.
        PALETTE: {},

        // ── DOM-ready gate (merged file runs at document-idle, but keep a guard) ──
        onReady(cb) {
        ...
        // ── ASP.NET WebForms plumbing (Student Attendance + Aggregator share this) ──
        aspnet: {
            token() {
            ...
            viewstate() { return this.field('__VIEWSTATE'); },
            // Folded features may add fetchForm/postback helpers as needed.
        },

        // ── Course-name dataset — populated when notificationBatch is folded in
        //    (the 294-course dataset lives in that source). Lazy to keep stubs light.
        courses: {
            map: null,
            lookup(code) { return (this.map && this.map[code]) || code; },
        },

        log(feature, ...args) { console.log(`[GIU-SS:${feature}]`, ...args); },
    ...
    const Features = {
        staffAttendance(S) {
            ...
            function createDayOffControlsRow(selectedDayCode, selectedDayFullName, onDayChange) {
                ...
                const effectiveDateInput = createUiInput("date", "giu-dayoff-effective-date", { value: getTodayLocalYMD() });
            ...
            function createConfigPanel(selectedDayCode, selectedDayFullName, periods, onDayChange, initialExpanded) {
                ...
                bodyInner.appendChild(createDayOffControlsRow(selectedDayCode, selectedDayFullName, onDayChange));
        ...
        proctorAggregator(S) {
            ...
            function toggleCoProctors(tr, row) {
                ...
                const cpItems = coProctors.map(r => {
                    ...
                    let badge = '';
                    if (isCovers)         badge = `<span class="gius-cover-status">Cover Proctor</span>`;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    const ROUTES = [
        // Wildcard pages (old scripts used @include portal/*):
        { id: 'staffAttendance',   test: () => true },
    ...
    function renderHomeFeatureToggles() {
        ...
        Shared.injectStyle('gius-feature-toggle-styles', `
            ...
            .gius-feature-panel .card{margin-bottom:20px;}
