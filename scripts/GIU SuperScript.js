...
// @description All-in-one GIU staff portal enhancements (9 features). Install this OR the individual scripts — never both.
// @author      Mo.Elmaadawy
    ...
    const Features = {
        ...
        unenteredSessions(S) {
            ...
            async function boot(opts = {}) {
                ...
                if (needEnumeration) {
                    const rawOptions = parseSessionOptions(doc);
                    const fresh = filterCandidates(rawOptions);
                    S.warn('unenteredSessions', 'DEBUG dropdown found:', !!doc.getElementById('MainContent_DDL_Sessions'),
                        'raw parsed:', rawOptions.length, 'fresh candidates:', fresh.length);
